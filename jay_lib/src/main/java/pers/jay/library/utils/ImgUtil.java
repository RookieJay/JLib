...
public class ImgUtil {
    ...
    public static Bitmap handleImageOnKitKat(Context context, Intent data) {
        String imagePath = getRealImagePath(context, data);
     ...
     */
    public static Bitmap returnRotatePhoto(String originpath, Bitmap originBmp) {

        // 取得图片旋转角度
        int angle = readPictureDegree(originpath);

        if (originBmp == null) {
            originBmp = BitmapFactory.decodeFile(originpath);
        }

        // 把原图压缩后得到Bitmap对象
        Bitmap bmp = ImageUtils.compressBySampleSize(originBmp, 60, true);
